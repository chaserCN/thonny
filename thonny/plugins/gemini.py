...
**Що далі:**
Після того, як ти збираєш оцінки, варто перевіряти, чи вони від 1 до 12. А ще, що робити, якщо хтось введе не число? Це зробить твою програму надійнішою.

Ось як можна змінити код у рядках 17-19, щоб він перевіряв оцінки:

```fix{lines:17-19}
    if оцінка_текст.isdigit():
        оцінка = int(оцінка_текст)
        # Додаємо перевірку, чи оцінка від 1 до 12
        if 1 <= оцінка <= 12:
            оцінки.append(оцінка)
        else:
            print("Оцінка має бути від 1 до 12.")
    else: # Якщо введене не число і не "готово"
        print("Будь ласка, введи число або 'готово'.")
```

Після того, як ми виправили збір оцінок, залишилось тільки порахувати середню оцінку та показати її. Цей код потрібно додати в кінець твого файлу, після циклу `while`:

```python
# Після циклу, рахуємо середню оцінку
if оцінки: # Перевіряємо, чи є оцінки у списку
    сума_оцінок = sum(оцінки)
    кількість_оцінок = len(оцінки)
    середня_оцінка = сума_оцінок / кількість_оцінок
    print(f"Твоя середня оцінка: {середня_оцінка:.2f}")
else:
    print("Ти не ввела жодної оцінки.")
```
